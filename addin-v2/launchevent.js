...
// ── QR code rendu en PNG (via Canvas) au lieu de SVG ──────────
// Outlook ne supporte pas fiablement les data-URI SVG dans le corps d'un email ;
// on dessine donc sur un <canvas> caché puis on exporte en PNG, largement supporté.
function buildInlineQrDataUrl(payload) {
  const moduleCount = 21;
  const size = 240; // rendu en plus grand pour rester net une fois réduit à 80px à l'affichage
  const quiet = 2;
