// ── Configuration ──────────────────────────────────────────────
// IMPORTANT : remplace par ton URL ngrok actuelle avant chaque test/demo
const API_URL = 'https://xxxx-xxx-xxx.ngrok-free.dev/api';
...
async function onNewMessageComposeHandler(event) {
  try {
    ...
    // 1. Essaie de récupérer le vrai template du Dashboard (centralisé)
    let html = await fetchSignatureFromDashboard(userEmail);

    if (html) {
      // 2. Injecte la photo + le QR code dans les placeholders du template
      html = await injectPhotoAndQr(html, userEmail, userName);
    } else {
      // 3. Fallback : si l'API est injoignable ou l'utilisateur n'existe pas encore en base
...
async function fetchSignatureFromDashboard(email) {
  try {
    const res = await fetch(
      API_URL + '/Signatures/by-email/' + encodeURIComponent(email) + '/active',
